...
LSM9DS1::LSM9DS1()
{
	init(IMU_MODE_I2C, LSM9DS1_AG_ADDR(1), LSM9DS1_M_ADDR(1), 1);
}
*/
...
uint16_t LSM9DS1::begin()
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)	// If we're using I2C
		initI2C(settings.device.agAddress, settings.device.bus);	// Initialize I2C
	//else if (settings.device.commInterface == IMU_MODE_SPI) 	// else, if we're using SPI
	//	initSPI();	// Initialize SPI
		
	// To verify communication, we can read from the WHO_AM_I register of
	// each device. Store those in a variable so we can return them.
	uint8_t mTest = mReadByte(WHO_AM_I_M, settings.device.bus);		// Read the gyro WHO_AM_I
	uint8_t xgTest = xgReadByte(WHO_AM_I_XG, settings.device.bus);	// Read the accel/mag WHO_AM_I
...
void LSM9DS1::initGyro()
{
	...
	xgWriteByte(CTRL_REG1_G, tempRegValue, settings.device.bus);
	
	// CTRL_REG2_G (Default value: 0x00)
	// [0][0][0][0][INT_SEL1][INT_SEL0][OUT_SEL1][OUT_SEL0]
	// INT_SEL[1:0] - INT selection configuration
	// OUT_SEL[1:0] - Out selection configuration
	xgWriteByte(CTRL_REG2_G, 0x00, settings.device.bus);	
	...
	xgWriteByte(CTRL_REG3_G, tempRegValue, settings.device.bus);
	...
	xgWriteByte(CTRL_REG4, tempRegValue, settings.device.bus);
	...
	xgWriteByte(ORIENT_CFG_G, tempRegValue, settings.device.bus);
}
...
void LSM9DS1::initAccel()
{
	...
	xgWriteByte(CTRL_REG5_XL, tempRegValue, settings.device.bus);
	...
	xgWriteByte(CTRL_REG6_XL, tempRegValue, settings.device.bus);
	...
	xgWriteByte(CTRL_REG7_XL, tempRegValue, settings.device.bus);
}
...
{  
	...
	while (samples < 0x1F)
	{
		samples = (xgReadByte(FIFO_SRC) & 0x3F, settings.device.bus); // Read number of stored samples
	}
...
void LSM9DS1::magOffset(uint8_t axis, int16_t offset)
{
	...
	msb = (offset & 0xFF00) >> 8;
	lsb = offset & 0x00FF;
	mWriteByte(OFFSET_X_REG_L_M + (2 * axis), lsb, settings.device.bus);
	mWriteByte(OFFSET_X_REG_H_M + (2 * axis), msb, settings.device.bus);
}
...
void LSM9DS1::initMag()
{
	...
	mWriteByte(CTRL_REG1_M, tempRegValue, settings.device.bus);
	...
	mWriteByte(CTRL_REG2_M, tempRegValue, settings.device.bus); // +/-4Gauss
	...
	mWriteByte(CTRL_REG3_M, tempRegValue, settings.device.bus); // Continuous conversion mode
	...
	mWriteByte(CTRL_REG4_M, tempRegValue, settings.device.bus);
	...
	tempRegValue = 0;
	mWriteByte(CTRL_REG5_M, tempRegValue, settings.device.bus);
}

uint8_t LSM9DS1::accelAvailable()
{
	uint8_t status = xgReadByte(STATUS_REG_1, settings.device.bus);
...
{
	uint8_t status;
	status = mReadByte(STATUS_REG_M, settings.device.bus);
...
void LSM9DS1::readAccel()
{
	...
	if ( xgReadBytes(OUT_X_L_XL, temp, 6, settings.device.bus) == 6 ) // Read 6 bytes, beginning at OUT_X_L_XL
	{
...
int16_t LSM9DS1::readAccel(lsm9ds1_axis axis)
{
	uint8_t temp[2];
	int16_t value;
	if ( xgReadBytes(OUT_X_L_XL + (2 * axis), temp, 2, settings.device.bus) == 2)
	{
...
void LSM9DS1::readMag()
{
	...
	if ( mReadBytes(OUT_X_L_M, temp, 6, settings.device.bus) == 6) // Read 6 bytes, beginning at OUT_X_L_M
	{
...
{
	uint8_t temp[2];
	if ( mReadBytes(OUT_X_L_M + (2 * axis), temp, 2, settings.device.bus) == 2)
	{
...
void LSM9DS1::readTemp()
{
	...
	if ( xgReadBytes(OUT_TEMP_L, temp, 2, settings.device.bus) == 2 ) // Read 2 bytes, beginning at OUT_TEMP_L
	{
...
void LSM9DS1::readGyro()
{
	...
	if ( xgReadBytes(OUT_X_L_G, temp, 6, settings.device.bus) == 6) // Read 6 bytes, beginning at OUT_X_L_G
	{
...
int16_t LSM9DS1::readGyro(lsm9ds1_axis axis)
{
	...
	if ( xgReadBytes(OUT_X_L_G + (2 * axis), temp, 2, settings.device.bus) == 2)
	{
		value = (temp[1] << 8) | temp[0];
...
void LSM9DS1::setGyroScale(uint16_t gScl)
{
	...
	uint8_t ctrl1RegValue = xgReadByte(CTRL_REG1_G, settings.device.bus);
	...
	xgWriteByte(CTRL_REG1_G, ctrl1RegValue, settings.device.bus);
...
void LSM9DS1::setAccelScale(uint8_t aScl)
{
	...
	uint8_t tempRegValue = xgReadByte(CTRL_REG6_XL, settings.device.bus);
...
void LSM9DS1::setMagScale(uint8_t mScl)
{
	...
	uint8_t temp = mReadByte(CTRL_REG2_M, settings.device.bus);
	...
	mWriteByte(CTRL_REG2_M, temp, settings.device.bus);
...
void LSM9DS1::setGyroODR(uint8_t gRate)
{
	...
	if ((gRate & 0x07) != 0)
	{
		...
		uint8_t temp = xgReadByte(CTRL_REG1_G, settings.device.bus);
		...
		xgWriteByte(CTRL_REG1_G, temp, settings.device.bus);
	}
}
...
void LSM9DS1::setAccelODR(uint8_t aRate)
{
	...
	if ((aRate & 0x07) != 0)
	{
		...
		uint8_t temp = xgReadByte(CTRL_REG6_XL, settings.device.bus);
		...
		xgWriteByte(CTRL_REG6_XL, temp, settings.device.bus);
	}
}
...
void LSM9DS1::setMagODR(uint8_t mRate)
{
	...
	uint8_t temp = mReadByte(CTRL_REG1_M, settings.device.bus);
	...
	mWriteByte(CTRL_REG1_M, temp, settings.device.bus);
}
	                     ...
	                     h_lactive activeLow, pp_od pushPull)
{
	...
	xgWriteByte(interrupt, generator, settings.device.bus);
	
	// Configure CTRL_REG8
	uint8_t temp;
	temp = xgReadByte(CTRL_REG8, settings.device.bus);
	...
	xgWriteByte(CTRL_REG8, temp, settings.device.bus);
}
...
void LSM9DS1::configInactivity(uint8_t duration, uint8_t threshold, bool sleepOn)
{
	...
	temp = threshold & 0x7F;
	if (sleepOn) temp |= (1<<7);
	xgWriteByte(ACT_THS, temp, settings.device.bus);
	
	xgWriteByte(ACT_DUR, duration, settings.device.bus);
}

uint8_t LSM9DS1::getInactivity()
{
	uint8_t temp = xgReadByte(STATUS_REG_0, settings.device.bus);
	temp &= (0x10);
	return temp;
}
...
void LSM9DS1::configAccelInt(uint8_t generator, bool andInterrupts)
{
	...
	xgWriteByte(INT_GEN_CFG_XL, temp, settings.device.bus);
}
...
void LSM9DS1::configAccelThs(uint8_t threshold, lsm9ds1_axis axis, uint8_t duration, bool wait)
{
	...
	xgWriteByte(INT_GEN_THS_X_XL + axis, threshold, settings.device.bus);
	
	// Write duration and wait to INT_GEN_DUR_XL
	uint8_t temp;
	temp = (duration & 0x7F);
	if (wait) temp |= 0x80;
	xgWriteByte(INT_GEN_DUR_XL, temp, settings.device.bus);
}

uint8_t LSM9DS1::getAccelIntSrc()
{
	uint8_t intSrc = xgReadByte(INT_GEN_SRC_XL, settings.device.bus);
...
void LSM9DS1::configGyroInt(uint8_t generator, bool aoi, bool latch)
{
	...
	if (aoi) temp |= 0x80;
	if (latch) temp |= 0x40;
	xgWriteByte(INT_GEN_CFG_G, temp, settings.device.bus);
}
...
void LSM9DS1::configGyroThs(int16_t threshold, lsm9ds1_axis axis, uint8_t duration, bool wait)
{
	...
	// axis will be 0, 1, or 2 (x, y, z respectively)
	xgWriteByte(INT_GEN_THS_XH_G + (axis * 2), buffer[0], settings.device.bus);
	xgWriteByte(INT_GEN_THS_XH_G + 1 + (axis * 2), buffer[1], settings.device.bus);
	
	// Write duration and wait to INT_GEN_DUR_XL
	uint8_t temp;
	temp = (duration & 0x7F);
	if (wait) temp |= 0x80;
	xgWriteByte(INT_GEN_DUR_G, temp, settings.device.bus);
}

uint8_t LSM9DS1::getGyroIntSrc()
{
	uint8_t intSrc = xgReadByte(INT_GEN_SRC_G, settings.device.bus);
...
void LSM9DS1::configMagInt(uint8_t generator, h_lactive activeLow, bool latch)
{
	...
	mWriteByte(INT_CFG_M, config, settings.device.bus);
}

void LSM9DS1::configMagThs(uint16_t threshold)
{
	// Write high eight bits of [threshold] to INT_THS_H_M
	mWriteByte(INT_THS_H_M, uint8_t((threshold & 0x7F00) >> 8), settings.device.bus);
	// Write low eight bits of [threshold] to INT_THS_L_M
	mWriteByte(INT_THS_L_M, uint8_t(threshold & 0x00FF), settings.device.bus);
}

uint8_t LSM9DS1::getMagIntSrc()
{
	uint8_t intSrc = mReadByte(INT_SRC_M, settings.device.bus);
...
{
	uint8_t temp = xgReadByte(CTRL_REG9, settings.device.bus);
	if (enable) temp |= (1<<6);
	else temp &= ~(1<<6);
	xgWriteByte(CTRL_REG9, temp, settings.device.bus);
}
...
void LSM9DS1::setFIFO(fifoMode_type fifoMode, uint8_t fifoThs)
{
	...
	xgWriteByte(FIFO_CTRL, ((fifoMode & 0x7) << 5) | (threshold & 0x1F), settings.device.bus);
}

uint8_t LSM9DS1::getFIFOSamples()
{
	return (xgReadByte(FIFO_SRC) & 0x3F, settings.device.bus);
}
...
void LSM9DS1::xgWriteByte(uint8_t subAddress, uint8_t data, uint8_t bus))
{
	// Whether we're using I2C or SPI, write a byte using the
	// gyro-specific I2C address or SPI CS pin.
	if (settings.device.commInterface == IMU_MODE_I2C)
		return I2CwriteByte(_xgAddress, subAddress, data, bus);
	//else if (settings.device.commInterface == IMU_MODE_SPI)
		//SPIwriteByte(_xgAddress, subAddress, data);
}

void LSM9DS1::mWriteByte(uint8_t subAddress, uint8_t data, uint8_t bus))
{
	// Whether we're using I2C or SPI, write a byte using the
	// accelerometer-specific I2C address or SPI CS pin.
	if (settings.device.commInterface == IMU_MODE_I2C)
		return I2CwriteByte(_mAddress, subAddress, data, bus);
	//else if (settings.device.commInterface == IMU_MODE_SPI)
		//return SPIwriteByte(_mAddress, subAddress, data);
}

uint8_t LSM9DS1::xgReadByte(uint8_t subAddress, uint8_t bus)
{
	// Whether we're using I2C or SPI, read a byte using the
	// gyro-specific I2C address or SPI CS pin.
	if (settings.device.commInterface == IMU_MODE_I2C)
		return I2CreadByte(_xgAddress, subAddress, bus);
	//else if (settings.device.commInterface == IMU_MODE_SPI)
		//return SPIreadByte(_xgAddress, subAddress);
	return -1;
}

uint8_t LSM9DS1::xgReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count, uint8_t bus)
{
	// Whether we're using I2C or SPI, read multiple bytes using the
	// gyro-specific I2C address or SPI CS pin.
	if (settings.device.commInterface == IMU_MODE_I2C)
		return I2CreadBytes(_xgAddress, subAddress, dest, count, bus);
	//else if (settings.device.commInterface == IMU_MODE_SPI)
		//return SPIreadBytes(_xgAddress, subAddress, dest, count);
	return -1;
}

uint8_t LSM9DS1::mReadByte(uint8_t subAddress, uint8_t bus)
{
	// Whether we're using I2C or SPI, read a byte using the
	// accelerometer-specific I2C address or SPI CS pin.
	if (settings.device.commInterface == IMU_MODE_I2C)
		return I2CreadByte(_mAddress, subAddress, bus);
	//else if (settings.device.commInterface == IMU_MODE_SPI)
		//return SPIreadByte(_mAddress, subAddress);
	return -1;
}

uint8_t LSM9DS1::mReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count, uint8_t bus)
{
	// Whether we're using I2C or SPI, read multiple bytes using the
	// accelerometer-specific I2C address or SPI CS pin.
	if (settings.device.commInterface == IMU_MODE_I2C)
		return I2CreadBytes(_mAddress, subAddress, dest, count, bus);
	//else if (settings.device.commInterface == IMU_MODE_SPI)
...
*/
void LSM9DS1::initI2C(uint8_t address, uint8_t I2CBus)
{
    ...
    if (ioctl(file, I2C_SLAVE, address) < 0){
            printf("I2C_SLAVE address %c failed...\n", address);
            return(2);
    }
    ...
    int numberBytes = 0x80;
    int bytesRead = read(file, dataBuffer, numberBytes);
    if (bytesRead == -1){
    	pritnf("Failure to read Byte Stream in readFullSensorState()");
    }
    close(file);

    if (this->dataBuffer[0]!=0x00){
    	printf("MAJOR FAILURE: DATA WITH BMA180 HAS LOST SYNC!");
    }	// Initialize I2C library
}

// Wire.h read and write protocols
void LSM9DS1::I2CwriteByte(uint8_t address, uint8_t subAddress, uint8_t data, uint8_t bus)
{
    ...
    if (ioctl(file, I2C_SLAVE, address) < 0){
            prinf("I2C_SLAVE address %c failed...", address);
            return(2);
    }
...
uint8_t LSM9DS1::I2CreadByte(uint8_t address, uint8_t subAddress, uint8_t bus)
{	/*
	...
	*/
	printf("Starting BMA180 I2C sensor state byte read");
    char namebuf[MAX_BUS];
   	snprintf(namebuf, sizeof(namebuf), "/dev/i2c-%d", bus);
    int file;
    if ((file = open(namebuf, O_RDWR)) < 0){
            printf("Failed to open BMA180 Sensor %s on I2C Bus", namebuf);
            return(1);
    }
    if (ioctl(file, I2C_SLAVE, address) < 0){
            printf("I2C_SLAVE address %c failed...", address);
            return(2);
    }
    // According to the BMA180 datasheet on page 59, you need to send the first address
    // in write mode and then a stop/start condition is issued. Data bytes are
...
uint8_t LSM9DS1::I2CreadBytes(uint8_t address, uint8_t subAddress, uint8_t * dest, uint8_t count, uint8_t bus)
{	/*
	byte retVal;
	Wire.beginTransmission(address);      // Initialize the Tx buffer
	...
	*/
	...
	for (int i=0; i<count;){
		dest[i++] = I2CreadByte(address, (subAddress + i), bus);
	}
	return count;
}
